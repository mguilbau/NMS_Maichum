void ProduceVnDist()
{
   TFile* file = TFile::Open("../../../VnVm/PbPb/output/vn_cent.root");
   TH1D* h = (TH1D*) file->Get("v2_cent");
   TF1* f = new TF1("f","pol4",0,3000);

   const int n = 15;
   double vn[n+2] = {0,15,20,50,150,250,340,450,560,700,860,1040,1250,1480,1650,2100};
   TH1D* hvn = new TH1D("hvn","hvn",n,vn);
   
   int bin = n;
   for(int i=0; i<h->GetNbinsX(); ++i)
   {
      hvn->SetBinContent(i+1,h->GetBinContent(bin));
      hvn->SetBinError  (i+1,h->GetBinError(bin));
      --bin;
   } 
   hvn->Fit(f);
   TF1* fit = hvn->GetFunction("f");
   //TF1* fit = new TF1("multSpectrum","exp(0.*x)",0.,3000.);
